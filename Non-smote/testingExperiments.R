...
randomforestfeatures = read.csv('randomForestFeaturesCopy.csv')
#randomforestfeatures = read.csv(file.choose())
fcres = read.csv('FC_plus_RES.csv')
features = c()
for(i in 1:length(randomforestfeatures[,5])){
  if(randomforestfeatures[i,5] > .1265){
...
for (i in 1:length(genes)) {
  ...
  svmmodel = svm(
    as.formula(bformula), training, cost=5
  )
...
write.csv(svmpred,"predictions_threshold_5000_cost.13per_withoutmfeheat.csv")
